            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h2 class="text-2xl font-semibold text-gray-800">Editar Pregunta</h2>
                    ...
                    <form action="{{ route('admin.questions.update', $question) }}" method="POST" id="questionForm">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <textarea name="question_text" id="question_text" rows="3" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>{{ old('question_text', $question->question_text) }}</textarea>
                            </div>
                            
                            <div>
                                <label for="problem_statement" class="block text-sm font-medium text-gray-700">Enunciado del Problema (opcional)</label>
                                <textarea name="problem_statement" id="problem_statement" rows="4" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('problem_statement', $question->problem_statement) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="explanation" id="explanation" rows="4" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('explanation', $question->explanation) }}</textarea>
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    <input type="text" name="youtube_id" id="youtube_id" value="{{ old('youtube_id', $question->youtube_id) }}" class="block w-full pr-10 sm:text-sm border border-gray-200 rounded-md px-3 py-2" placeholder="Ej: dQw4w9WgXcQ o https://www.youtube.com/watch?v=dQw4w9WgXcQ">
                                ...
                                <div id="options-container">
                                    ...
                                    <div class="option-item mb-2 p-3 border border-gray-300 rounded-md">
                                        <div class="flex items-center">
                                            <input type="radio" name="correct_option" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" value="{{ $index }}" {{ $option->is_correct ? 'checked' : '' }}>
                                            <input type="hidden" name="options[{{ $index }}][id]" value="{{ $option->id }}">
                                            <div class="ml-2 flex-grow">
                                                <input type="text" name="options[{{ $index }}][text]" value="{{ old('options.' . $index . '.text', $option->option_text ?? $option->text ?? '') }}" class="mt-1 block w-full shadow-sm sm:text-sm border border-gray-200 rounded-md px-3 py-2" placeholder="Opción {{ $index + 1 }}" required>
                                            </div>
                                            @if($index > 1)
